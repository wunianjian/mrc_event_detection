...
def preprocessing(data_type, files):
    ...
    with open('output/{}_process.json'.format(data_type), 'w') as f:
...
def re_arrange(data):
    process_data_list = []
    for inst in data:
        event_mentions = deepcopy(inst['golden-event-mentions'])
        entities = inst['golden-entity-mentions']
        event_labels = ['Other'] * len(inst['words'])
        for event_mention in event_mentions:
            trigger = event_mention['trigger']
            event_labels[trigger['start']] = 'B-' + event_mention['event_type']
            for i in range(trigger['start'] + 1, trigger['end']):
                event_labels[i] = 'I-' + event_mention['event_type']
            arguments = ['None'] * len(entities)
            for argument in event_mention['arguments']:
                for idx, entity in enumerate(entities):
                    if (argument['start'], argument['end']) == (entity['start'], entity['end']):
                        arguments[idx] = argument['role']
                        break
                else:
                    print('error!')
        ...
        process_data_list.append({
            ...
            'words': inst['words'],
            'event-mentions': event_mentions,
            'entities': entities,
            'event-labels': event_labels,
            'stanford-colcc': inst['stanford-colcc'],
            'pos-tags': inst['pos-tags']
        })
    ch_times = 0
    for process_data in process_data_list:
        if is_chinese(''.join(process_data['words'])):
            ch_times += 1
            continue
    print('ch_times:', ch_times)
